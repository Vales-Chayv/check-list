    ...
    function stickerHTML(s, si) {
     ...
     return `<div style="display:flex;justify-content:${align};margin:16px 0 4px">
        <div style="max-width:88%;min-width:55%">
         ...
         <!-- Bottom peek + content -->
          ${hasFiles?`
            <div style="height:18px;background:${bottomColor};border-radius:0 0 8px 8px;transform:rotate(-1.5deg);transform-origin:left;margin-top:-6px;margin-left:-4px;margin-right:-4px"></div>
            <div id="${stkId}" style="display:none;background:${bottomColor};border-radius:0 0 10px 10px;padding:12px 14px;margin-left:-4px;margin-right:-4px">${filesHTML}</div>
          `:''}
        </div>
      </div>`;
    }
